...
为了提升多语言用户的使用体验，本项目实现了完整的国际化框架，并完成了全界面的中英文适配：

*   **核心实现**:
    *   **i18n 工具类 (`src/utils/i18n.js`)**: 封装了 `t()` 翻译函数和语言切换逻辑。支持基于字典的即时翻译。
    *   **状态持久化**: 语言偏好存储在 `localStorage` 中，并通过 `src/utils/settings.js` 进行管理，默认语言设为中文 (`zh`)。
*   **多语言适配范围**:
    *   **基础布局**: 侧边栏导航、搜索框、全局设置、登录界面。
    *   **播放控制**: 底部播放器、全屏播放页（包含播放队列、倍速控制、歌曲信息滚动提示等）。
    *   **数据展示**: 歌曲列表表单头、专辑详情信息、搜索结果分类。
    *   **交互反馈**: 删除确认弹窗、新建/编辑播放列表弹窗、加载提示等。
*   **UI 交互**: 在“设置”页面集成了语言切换按钮组，支持在“中文”与“English”之间无缝切换。切换后通过自动刷新应用全局生效。
...
为了方便用户浏览完整的音乐库，本项目实现了“所有歌曲”的分页查询功能：

*   **API 实现 (`src/api/subsonicApi.js`)**:
    *   由于 Subsonic 标准协议缺少直接获取全量歌曲的接口，本项目通过封装 `search3` 接口并传入通配符 `query="*"` 实现了该功能。
    *   支持 `songCount` (每页条数) 和 `songOffset` (偏移量) 参数进行服务端分页，极大提升了大数据量下的加载性能。
*   **视图组件 (`src/components/AllSongsView/`)**:
    *   **分页逻辑**: 实现了基于状态 (`page`, `count`) 的动态加载。通过判断返回结果长度是否小于请求长度来自动处理“下一页”按钮的禁用状态。
    *   **交互设计**: 
        *   提供了“上一页”和“下一页”导航按钮。
        *   集成了 `SelectPicker` 允许用户切换每页显示的歌曲数量 (5, 10, 15, 20, 30)。
        *   复用了 `SongsTableEnhanced` 组件，保持与搜索结果一致的展示字段（标题、艺术家、专辑、时长等）。
*   **响应式布局优化**:
    *   **移动端适配**: 在手机端自动精简分页栏文案（隐藏“每页歌曲数”标签），并使用小型化 (`size="sm"`) 组件，确保分页选择器与翻页按钮在窄屏下不换行。
    *   **布局统一**: 无论 PC 还是移动端，分页控制栏统一放置在列表下方，符合现代 Web 应用的交互习惯。
*   **系统集成**:
    *   **入口配置**: 在侧边栏“音乐库”顶部新增了“所有歌曲”选项卡，并关联了 `music` 图标。
    *   **国际化**: 同步完成了“所有歌曲”、“每页歌曲数”、“上一页/下一页”等相关词条的中英文翻译适配。
